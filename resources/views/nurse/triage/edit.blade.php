        ...
        <div class="card border-0 rounded-3 mb-4">
            <div class="card-header bg-secondary text-white p-4" id="priorityHeader">
                <h6 class="mb-0 fw-semibold d-flex align-items-center">
                    ...
                    Priority: <span id="overallPriorityText" class="ms-2">{{ $vitalSign->overall_priority }}</span>
                </h6>
            </div>
            <div class="card-body p-4">
                <p class="text-muted mb-0 small">Priority is automatically recalculated based on vital signs values.</p>
            </div>
        </div>
    </div>
        ...
        <div class="card border-0 rounded-3">
            <div class="card-header bg-white border-bottom p-4">
                <h5 class="mb-0 fw-semibold">
                    <span class="material-symbols-outlined me-2 align-middle">edit</span>
                    Edit Vital Signs
                </h5>
            </div>
            <div class="card-body p-4">
                <form action="{{ route('nurse.triage.update', $encounter) }}" method="POST" id="triageForm">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" step="0.1" name="temperature" id="temperature" 
                                       ...
                                       value="{{ old('temperature', $vitalSign->temperature) }}" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" name="spo2" id="spo2" 
                                       ...
                                       value="{{ old('spo2', $vitalSign->spo2) }}" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" name="blood_pressure_systolic" id="bpSystolic" 
                                       ...
                                       value="{{ old('blood_pressure_systolic', $vitalSign->blood_pressure_systolic) }}" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" name="blood_pressure_diastolic" id="bpDiastolic" 
                                       ...
                                       value="{{ old('blood_pressure_diastolic', $vitalSign->blood_pressure_diastolic) }}" required>
                                <span class="input-group-text">mmHg</span>
                            </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" name="pulse_rate" id="pulseRate" 
                                       ...
                                       value="{{ old('pulse_rate', $vitalSign->pulse_rate) }}" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" name="respiration_rate" id="respirationRate" 
                                       ...
                                       value="{{ old('respiration_rate', $vitalSign->respiration_rate) }}" required>
                    ...
                    <input type="hidden" name="overall_priority" id="overallPriority" value="{{ $vitalSign->overall_priority }}">

                    <div class="d-flex justify-content-between mt-4">
                        <a href="{{ route('nurse.triage.show', $encounter) }}" class="btn btn-outline-secondary">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function getTemperatureStatus(temp) {
        if (temp >= 36.0 && temp <= 37.5) return { label: 'Normal', color: '#0d6efd', priority: 'Normal' };
        else if (temp >= 37.6 && temp <= 38.5) return { label: 'Moderate', color: '#fd7e14', priority: 'Warning' };
        else if (temp >= 38.6 || temp < 35.0) return { label: 'Critical', color: '#dc3545', priority: 'Critical' };
        return { label: 'Low', color: '#0d6efd', priority: 'Normal' };
    }

    function getSpO2Status(spo2) {
        if (spo2 >= 95 && spo2 <= 100) return { label: 'Normal', color: '#0d6efd', priority: 'Normal' };
        else if (spo2 >= 90 && spo2 <= 94) return { label: 'Moderate', color: '#fd7e14', priority: 'Warning' };
        else if (spo2 < 90) return { label: 'Critical', color: '#dc3545', priority: 'Critical' };
        return { label: 'Invalid', color: '#6c757d', priority: 'Normal' };
    }

    function getBPStatus(systolic, diastolic) {
        if (systolic < 90 || diastolic < 60) return { label: 'Low - Critical', color: '#dc3545', priority: 'Critical' };
        else if (systolic < 120 && diastolic < 80) return { label: 'Normal', color: '#0d6efd', priority: 'Normal' };
        else if ((systolic >= 120 && systolic <= 139) || (diastolic >= 80 && diastolic <= 89)) return { label: 'Elevated', color: '#fd7e14', priority: 'Warning' };
        else if (systolic >= 140 || diastolic >= 90) return { label: 'High - Critical', color: '#dc3545', priority: 'Critical' };
        return { label: 'Unknown', color: '#6c757d', priority: 'Normal' };
    }

    function getPulseStatus(pulse) {
        if (pulse >= 60 && pulse <= 100) return { label: 'Normal', color: '#0d6efd', priority: 'Normal' };
        else if ((pulse >= 50 && pulse <= 59) || (pulse >= 101 && pulse <= 120)) return { label: 'Moderate', color: '#fd7e14', priority: 'Warning' };
        else if (pulse < 50 || pulse > 120) return { label: 'Critical', color: '#dc3545', priority: 'Critical' };
        return { label: 'Invalid', color: '#6c757d', priority: 'Normal' };
    }

    function getRespirationStatus(resp) {
        if (resp >= 12 && resp <= 20) return { label: 'Normal', color: '#0d6efd', priority: 'Normal' };
        else if ((resp >= 10 && resp <= 11) || (resp >= 21 && resp <= 25)) return { label: 'Moderate', color: '#fd7e14', priority: 'Warning' };
        else if (resp < 10 || resp > 25) return { label: 'Critical', color: '#dc3545', priority: 'Critical' };
        return { label: 'Invalid', color: '#6c757d', priority: 'Normal' };
    }
    ...
    function initializeStatuses() {
        const temp = parseFloat(fields.temperature.value);
        if (temp) {
            statuses.temperature = getTemperatureStatus(temp);
            statusElements.temperature.innerHTML = createStatusBadge(statuses.temperature.label, statuses.temperature.color);
        }
        
        const spo2 = parseInt(fields.spo2.value);
        if (spo2) {
            statuses.spo2 = getSpO2Status(spo2);
        ...
        const pulse = parseInt(fields.pulseRate.value);
        if (pulse) {
            statuses.pulse = getPulseStatus(pulse);
            statusElements.pulse.innerHTML = createStatusBadge(statuses.pulse.label, statuses.pulse.color);
        }
        
        const resp = parseInt(fields.respirationRate.value);
        if (resp) {
            statuses.respiration = getRespirationStatus(resp);
