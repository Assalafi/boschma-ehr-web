...
.rad-card{background:#fff;border-radius:14px;border:1px solid var(--lb);box-shadow:0 1px 3px rgba(0,0,0,.04);overflow:hidden;margin-bottom:20px}
.rad-card-header{padding:14px 20px;font-weight:600;font-size:13px;border-bottom:1px solid var(--lb);display:flex;align-items:center;gap:8px;color:#1e293b}
.rad-card-header .material-symbols-outlined{font-size:18px;color:var(--rad)}
.rad-card-body{padding:16px 20px}
...
.rad-badge-red{background:#fee2e2;color:#991b1b}
...
.rad-btn-success{background:#10b981;color:#fff}.rad-btn-success:hover{background:#059669;color:#fff}
.rad-btn-amber{background:#fef3c7;color:#92400e;border:1.5px solid #fcd34d}.rad-btn-amber:hover{background:#fde68a}
.rad-btn-blue{background:#dbeafe;color:#1e40af;border:1.5px solid #93c5fd}.rad-btn-blue:hover{background:#bfdbfe}
...
.form-control:focus,.form-select:focus{border-color:var(--rad);box-shadow:0 0 0 3px rgba(124,58,237,.1)}
...
<div class="rad-header d-flex justify-content-between align-items-center flex-wrap gap-2">
    <h4>Radiology Order — {{ $item->serviceItem?->name ?? 'Study' }}</h4>
    <nav style="--bs-breadcrumb-divider:'>';" aria-label="breadcrumb">
        <ol class="breadcrumb align-items-center mb-0 lh-1" style="font-size:12px">
            ...
            <li class="breadcrumb-item"><a href="{{ route('radiologist.queue') }}">Worklist</a></li>
...
<div class="col-lg-4">
    <div class="rad-card">
        <div class="rad-card-header"><span class="material-symbols-outlined">person</span> Patient Information</div>
    ...
    @if($enc)
    <div class="rad-card">
        <div class="rad-card-header"><span class="material-symbols-outlined">event_note</span> Encounter</div>
        <div class="rad-card-body">
            <div class="info-row"><span class="info-label">Visit</span><span class="info-value">{{ $enc->visit_date?->format('d M Y') }}</span></div>
            <div class="info-row"><span class="info-label">Status</span><span class="info-value"><span class="rad-badge rad-badge-gray">{{ $enc->status }}</span></span></div>
    ...
    <div class="rad-card">
        <div class="rad-card-header"><span class="material-symbols-outlined">monitor_heart</span> Latest Vitals</div>
        <div class="rad-card-body">
...
<div class="col-lg-8">
    <div class="rad-card">
        <div class="rad-card-header d-flex justify-content-between w-100">
            <div class="d-flex align-items-center gap-2"><span class="material-symbols-outlined">radiology</span> {{ $item->serviceItem?->name??'Radiology Study' }}</div>
        ...
        <div class="rad-card-body">
            ...
            <div class="d-flex gap-2 pt-3" style="border-top:1px solid #f1f5f9">
                ...
                <form method="POST" action="{{ route('radiologist.order.status',$item) }}">@csrf<input type="hidden" name="status" value="in_progress">
                    <button class="rad-btn rad-btn-blue"><span class="material-symbols-outlined" style="font-size:15px">radiology</span> Mark In Progress</button>
                </form>
                @elseif($item->status==='in_progress')
                <form method="POST" action="{{ route('radiologist.order.status',$item) }}">@csrf<input type="hidden" name="status" value="pending">
                    <button class="rad-btn rad-btn-amber"><span class="material-symbols-outlined" style="font-size:15px">undo</span> Revert Pending</button>
                </form>
                @endif
                <a href="{{ route('radiologist.queue') }}" class="rad-btn rad-btn-outline ms-auto">Back to Worklist</a>
            </div>
        </div>
    </div>
    ...
    <div class="rad-card">
        <div class="rad-card-header" style="color:#166534"><span class="material-symbols-outlined" style="color:#10b981">verified</span> Recorded Reports</div>
        @foreach($item->serviceResults as $res)
        <div class="result-entry">
            <div class="d-flex justify-content-between align-items-center mb-2">
                <span class="rad-badge rad-badge-green"><span class="material-symbols-outlined" style="font-size:12px">check_circle</span> Report #{{ $loop->iteration }}</span>
                <span class="text-muted" style="font-size:11px">{{ $res->reported_at?->format('d M Y H:i') }} — {{ $res->reportedBy?->name??'—' }}</span>
            </div>
            @if($res->result_value)
            <div class="mb-1" style="font-size:13px"><span class="info-label">Findings:</span> <strong style="color:#1e293b">{{ $res->result_value }}</strong> @if($res->reference_range)<span class="text-muted" style="font-size:11px">(Ref: {{ $res->reference_range }})</span>@endif</div>
            @endif
            @if($res->remark)
            <div class="mb-1" style="font-size:13px"><span class="info-label">Impression:</span> <span class="rad-badge rad-badge-{{ in_array($res->remark,['Normal','Negative'])?'green':'red' }}">{{ $res->remark }}</span></div>
            @endif
            ...
            <div class="mt-2 d-flex gap-1 flex-wrap">
                ...
                <a href="{{ Storage::url($document) }}" target="_blank" class="rad-btn rad-btn-outline" style="font-size:11px;padding:4px 10px">
    ...
    <div class="rad-card">
        <div class="rad-card-header"><span class="material-symbols-outlined">add_circle</span> Record Radiology Report</div>
        <div class="rad-card-body">
            <form method="POST" action="{{ route('radiologist.order.result',$item) }}" enctype="multipart/form-data">
                @csrf
                <div class="row g-3">
                    <div class="col-md-6">
                        <label class="form-label">Findings / Result Value</label>
                        <input type="text" name="result_value" class="form-control @error('result_value') is-invalid @enderror" value="{{ old('result_value') }}" placeholder="e.g. No acute cardiopulmonary findings">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Reference / Normal Range</label>
                        <input type="text" name="reference_range" class="form-control" value="{{ old('reference_range') }}" placeholder="e.g. Normal limits">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Impression</label>
                        <select name="remark" class="form-select">
                            <option value="">— Select —</option>
                            @foreach(['Normal','Abnormal','Critical','Positive','Negative','Borderline','Inconclusive'] as $r)
                            <option value="{{ $r }}" {{ old('remark')===$r?'selected':'' }}>{{ $r }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-12">
                        <label class="form-label">Report Documents <span class="text-muted fw-normal">(PDF/Image, max 5MB per file)</span></label>
                        <div id="fileUploadContainer" style="border:1.5px dashed var(--lb);border-radius:12px;padding:16px;background:#f8fafc">
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <span style="font-size:12px;color:#64748b">Upload imaging reports or scanned documents</span>
                                <button type="button" class="rad-btn rad-btn-outline" style="font-size:11px;padding:4px 10px" onclick="addFileInput()">
                                ...
                                <div class="file-input-group mb-2">
                                    ...
                                    <div class="file-preview mt-2" style="display:none">
                                        <div class="d-flex align-items-center gap-2 p-2 bg-white rounded-3 border">
                                            <span class="material-symbols-outlined" style="color:var(--rad);font-size:18px">description</span>
                                            <span class="file-name" style="font-size:12px;color:#1e293b"></span>
                                            <button type="button" class="rad-btn ms-auto" style="background:#fee2e2;color:#991b1b;font-size:11px;padding:3px 8px" onclick="removeFileInput(this)">
                    ...
                    <div class="col-12">
                        <label class="form-label">Clinical Interpretation / Notes</label>
                        <textarea name="result_note" rows="4" class="form-control @error('result_note') is-invalid @enderror" placeholder="Detailed radiological findings, clinical impression, recommendations…">{{ old('result_note') }}</textarea>
                    </div>
                    <div class="col-12 d-flex gap-2">
                        <button type="submit" class="rad-btn rad-btn-success">
                            <span class="material-symbols-outlined" style="font-size:16px">save</span> Save Report & Complete
                        </button>
                        <a href="{{ route('radiologist.queue') }}" class="rad-btn rad-btn-outline">Cancel</a>
                    </div>
                </div>
            </form>
        </div>
...
function addFileInput(){
    ...
    d.innerHTML=`<input type="file" name="result_files[]" class="form-control" accept=".pdf,.jpg,.jpeg,.png" onchange="previewFile(this)"><div class="file-preview mt-2" style="display:none"><div class="d-flex align-items-center gap-2 p-2 bg-white rounded-3 border"><span class="material-symbols-outlined" style="color:var(--rad);font-size:18px">description</span><span class="file-name" style="font-size:12px;color:#1e293b"></span><button type="button" class="rad-btn ms-auto" style="background:#fee2e2;color:#991b1b;font-size:11px;padding:3px 8px" onclick="removeFileInput(this)"><span class="material-symbols-outlined" style="font-size:14px">close</span></button></div></div>`;
    c.appendChild(d);
}
